...
import { functionCallToString, sendMessageFn, waitForSelectorWebview, waitForTextWebview } from './utils';
...
export interface WebviewHandlers {
	...
	subscribe<S>(event: string, listener: (data: S) => void, onEvent: string, selector?: string): void;
...
export type WebviewProps = Partial<WebViewProps> & { ref: (instance: WebView) => void };

export function useWebview(): [WebviewProps, WebviewHandlers] {
	const [resolvers] = useState(() => {
		return new Map<string, { resolve: (data?: unknown) => void; retry: () => void; persist?: boolean }>();
	});
	const [isLoaded, setIsLoaded] = useState(false);
	const [initialize, setInitialize] = useState<null | (() => void)>(null);
	const [initialized] = useState(() => new Promise(resolve => setInitialize(() => resolve)));
	...
	const addEventListener = useCallback(
		function addEventListenerFn<E, D>(key: string, event: string, onEvent: string, selector?: string) {
	...
	const subscribe: WebviewHandlers['subscribe'] = useCallback(
		(event, listener, onEvent, selector) => {
			...
			resolvers.set(key, { resolve: listener as (data: unknown) => void, retry, persist: true });
