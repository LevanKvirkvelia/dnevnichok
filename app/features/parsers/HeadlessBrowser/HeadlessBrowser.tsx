...
import {useControlledWebview, WebviewHandlers} from './useControlledWebview';
import {defaultUserAgent} from './utils';
import {useTheme} from '../../themes/useTheme';
import {ProgressBar} from '../../browser/components/ProgressBar';
import {CloseButton} from '../../../ui/CloseButton';

const HeadlessBrowserContext = React.createContext<HeadlessBrowserHandlers | null>(null);

const {width, height} = Dimensions.get('screen');

export type HeadlessBrowserHandlers<T extends boolean = true> = WebviewHandlers & {
  close(): void;
...
export function useHeadlessBrowser<T extends boolean>({headless = true as T}: HeadlessBrowserParams<T> = {}) {
  const ctx = useContext(HeadlessBrowserContext) as HeadlessBrowserHandlers<T> | null;
...
export function HeadlessBrowserProvider({children}: {children: React.ReactNode}) {
  ...
  const [onUrlChangeCb, setOnUrlChangeCb] = useState<((url: string) => void) | null>(null);
  const [onRequestCb, setOnRequestCb] = useState<((url: string) => boolean) | null>(null);
  const [onErrorCb, setOnErrorCb] = useState<((error: Error) => void) | null>(null);
  ...
  const [webviewProps, webviewHandlers] = useControlledWebview();
  ...
  const onClose: HeadlessBrowserHandlers['onClose'] = useCallback(cb => (onCloseCbRef.current = cb), []);
  const onRequest: HeadlessBrowserHandlers['onRequest'] = useCallback(cb => setOnRequestCb(() => cb), []);
  const onError: HeadlessBrowserHandlers['onError'] = useCallback(cb => setOnErrorCb(() => cb), []);
  const onUrlChange: HeadlessBrowserHandlers['onUrlChange'] = useCallback(cb => setOnUrlChangeCb(() => cb), []);
  const setForcedVisibility: HeadlessBrowserHandlers['setForcedVisibility'] = useCallback(
    (v: boolean) => setForceVisibility(v),
    [],
  );
  ...
  const webview = uri ? (
    <View style={{flex: 1}}>
      <ProgressBar color={colors.primary} backgroundColor="white" loading={inProgress} progress={progress} />
      <WebView
        key={uri}
        ...
        onShouldStartLoadWithRequest={({url}) => (onRequestCb ? onRequestCb(url) : requestsEnabled)}
        onLoadProgress={({nativeEvent}) => setProgress(nativeEvent.progress)}
        onHttpError={({nativeEvent}) =>
          onErrorCb ? onErrorCb(new Error(`HTTP error ${nativeEvent.statusCode}. ${nativeEvent.url}`)) : null
        }
